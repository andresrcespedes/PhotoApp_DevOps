...
@pytest.mark.asyncio
@pytest.mark.usefixtures("clearFilters")
...
async def test_post_twice():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response1 = await ac.post('/filters',
                                  headers=headers_content,
                                  content=json.dumps(data1))
        assert response1.status_code == 201

        response2 = await ac.post('/filters',
                                  headers=headers_content,
                                  content=json.dumps(data1))
...
@pytest.mark.usefixtures("clearFilters")
@pytest.mark.usefixtures("initDB")
async def test_has_more_false_filters():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post('/filters',
                           headers=headers_content,
                           content=json.dumps(data1))
        ...
        response2 = await ac.get('/filters?offset=0&limit=10')
...
@pytest.mark.usefixtures("clearFilters")
@pytest.mark.usefixtures("initDB")
async def test_has_more_true_filters():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response1 = await ac.post('/filters',
                                  headers=headers_content,
                                  content=json.dumps(data1))
        ...
        response3 = await ac.get('/filters?offset=0&limit=1')
...
async def test_delete_filter():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        response2 = await ac.delete(f'/filter/{data1["display_name"]}',
                                  headers=headers_content)
        assert response2.status_code == 200
...
async def test_delete_empty():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response1 = await ac.delete(f'/filter/{data1["display_name"]}',
                                  headers=headers_content)
